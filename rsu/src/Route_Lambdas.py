...
# Uses dataframe obtained from the local speeds data and historical data
...
def get_travel_time_from_database(start, end, attr, sensor_data=None, time_window=None):
    # Check first in the "real_data" key for the tmc_id
    # real_data = sensor_data['real_data']
    # Check next in the "hist_data" for the tmc_id if not present in the real_data
    ...
    if tmc_id and length:
        if tmc_id in hist_data['tmc_id'].unique():
            ...
            mongodb = MyMongoDBClass(host="mongo", db="admin")
            ...
            collection = mongodb._db[GRID_ID]
